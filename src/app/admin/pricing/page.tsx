...
const iconOptions: { value: string; label: string; Icon: LucideIcon }[] = [
  { value: 'Tag', label: 'タグ', Icon: Tag },
  { value: 'Clock', label: '時間', Icon: Clock },
  { value: 'PlusCircle', label: 'オプション', Icon: PlusCircle },
  { value: 'Sparkles', label: 'キラキラ', Icon: Sparkles },
  { value: 'Star', label: 'スター', Icon: Star },
];
...
export default function DrinkMenuEditor() {
  ...
  const [editingDrink, setEditingDrink] = useState<MenuItem | null>(null);
  ...
  // 新規カテゴリフォーム
  const [newCategory, setNewCategory] = useState({ name: '', name_en: '', color: '#6366f1', icon: 'Tag' });
  ...
  const addCategory = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setNewCategory({ name: '', name_en: '', color: '#6366f1', icon: 'Tag' });
  ...
  // 削除
  const deleteItem = async (type: 'category' | 'item', id: string, name: string) => {
    const message = type === 'category'
      ? `「${name}」カテゴリとその中の項目を全て削除しますか？`
      : `「${name}」を削除しますか？`;

    if (!confirm(message)) return;

    try {
      const res = await fetch(`/api/menu?type=${type}&id=${id}`, {
        method: 'DELETE'
      });
      if (res.ok) {
        fetchData();
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <AdminHeader title="料金メニュー編集" backHref="/admin" />
      ...
      {categories.length === 0 ? (
        <div className="cast-card p-8 text-center">
          <p style={{ color: 'var(--accent-600)' }}>
            まだカテゴリがありません。「カテゴリ追加」ボタンから登録しましょう。
          </p>
        </div>
      ) : (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                      ...
                      {editingDrink?.id === drink.id ? (
                        ...
                        <>
                          ...
                          <div className="flex gap-2">
                            ...
                            <button
                              onClick={() => deleteItem('item', drink.id, drink.name)}
                  ...
                  {newDrinkCategory === category.id ? (
                    ...
                    >
                      ...
                      <span className="font-bold">項目追加</span>
                    </button>
                  )}
                </div>
              </div>
