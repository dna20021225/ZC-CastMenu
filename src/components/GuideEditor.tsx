 ...
 *
 * 注意（過去の上書きバグへの対策）:
 * 旧版は useCallback の依存配列に props を入れたため、
 * 親の再レンダ → fetchSetting 再生成 → useEffect 再発火 → 編集中テキストが
 * DB の値で上書きされる事故があった（PR #14 → リバート c6c40a4）。
 * 本実装では useEffect の依存配列を [] に固定し、初回マウント時のみ fetch する。
 */
export function GuideEditor({
  ...
  // 初回 fetch が終わるまで textarea を disabled にして、編集中に fetch が値を上書きする罠を避ける。
  const [loaded, setLoaded] = useState(false);
  const [saving, setSaving] = useState(false);
  const [feedback, setFeedback] = useState<{ type: 'success' | 'error'; message: string } | null>(null);

  // 初回マウント時のみ fetch。依存配列は空に固定する。
  // settingsKey や defaultText は props だが、編集セッション中に
  // 同じインスタンスで切り替わることはないので [] でよい。
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        ...
        if (!cancelled) setText(defaultText);
      } finally {
  ...
  return (
    <div className="cast-card p-5">
      ...
      <textarea
        ...
        disabled={!loaded}
        className="w-full px-3 py-2 rounded-md text-sm font-mono disabled:opacity-50"
        style={{
      ...
      <div className="flex items-center gap-3 mt-3">
        ...
        {!loaded && <span className="text-sm" style={{ color: 'var(--secondary)' }}>読み込み中…</span>}
      </div>
    </div>
  );
}
